...
class RegistrationController extends AbstractController
{
    ...
    public function register(Request $request, UserPasswordEncoderInterface $passwordEncoder, GuardAuthenticatorHandler $guardHandler, AppAuthenticator $authenticator, TranslatorInterface $translator): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            //check si le tableau user est null
            $check = $entityManager->getRepository(User::class)->findAll();

            //si le tableau est null, le premier inscrit obtient tous les droits
            if($check == null)
            {
                ...
                //attribution du ROLE_SUPER_ADMIN
                $user->setRoles(['ROLE_USER', 'ROLE_ADMIN', 'ROLE_SUPER_ADMIN']);
                //défini la date d'inscription
                $user->setDateRegister(new \DateTime()); // \DateTime() variable PHP
            ...
            //inscription normale pour les autres
            else{
                // encode the plain password
                $user->setPassword(
                    $passwordEncoder->encodePassword(
